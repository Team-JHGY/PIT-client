  ...
  return (
    <>
      <Appbar.Header style={globalStyle.appbarMain}>
        ...
        <Pressable style={[globalStyle.appbarBtnArrow]}>
          ...
          <ScrollView>
            {userData.map((item, index) => {
              return (
                <Pressable
                  key={index}
                  onPress={() => navigation.navigate('MemberRoom', { memberInfo: item })}
                >
                  <View style={[globalStyle.row, styles.userInfo]}>
                    <View>
                      {item.member.user.profileImage !== null ? (
                        <Image
                          source={{ uri: item.member.user.profileImage.path }}
                    ...
                    <View style={globalStyle.col_2}>
                      <Text style={[globalStyle.body2, styles.textmargin]}>
                        ...
                        (
                        {userInfo[0].type === 'TRAINER'
                          ? item.member.gender === 'MAN'
                            ? '남'
                            : '여'
                          : item.trainer.gender === 'MAN'
                          ? '남'
                          : '여'}
                        ,
                        {userInfo[0].type === 'TRAINER'
                          ...
                          : new Date().getFullYear() -
                            new Date(item.trainer.birthday).getFullYear()}
                        세 )
                      </Text>
                      <Text
                      ...
                      >
                        {userInfo[0].type === 'TRAINER'
                          ? item.member.modifedAt === null
                            ? 'null'
                            : item.member.modifedAt.split('T')[0].replace(/-/gi, '.')
                          : item.trainer.modifedAt === null
                          ? 'null'
                          : item.trainer.modifedAt.split('T')[0].replace(/-/gi, '.')}
                      </Text>
                    </View>
                    {userInfo[0].type === 'TRAINER' ? (
                      item.member.modifedAt !== null &&
                      ...
                      new Date().getDate() === new Date(item.member.modifedAt).getDate() ? (
                        <>
                          ...
                          >
                            {item.member.modifedAt === null
                              ? 'null'
                              : item.member.modifedAt.split('T')[0].replace(/-/gi, '.').slice(5)}
                          </Text>
                        </>
                      ) : (
                      ...
                      <>
                        ...
                        >
                          {item.trainer.modifedAt === null
                            ? 'null'
                            : item.trainer.modifedAt.split('T')[0].replace(/-/gi, '.').slice(5)}
                        </Text>
                      </>
                    ) : (
                      <Text style={[globalStyle.body2, globalStyle.textDimmedGrey, styles.date]}>
